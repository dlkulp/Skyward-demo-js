function randBetween(a: number, b: number, int: boolean = false): number {
    if (int == false) {
...
class Obj {
    ...
    public tick(input?: Input, astar?: any): void {
...
class Phone extends Interactable {
    public tick() {
...
class PickUp extends Interactable {
    ...
    constructor(x: number, y: number, z: number, weight?: number) {
        super(x, y, z); // Dunno what all we want here
    }
}

class HandGun extends PickUp {
    constructor(x: number, y: number, z: number, weight: number) {
        super(x, y, z);
...
function gridToScreen(x: number, y: number) {
    var out = new Vector2(600 + 32 * x - 32 * y, 16 * x + 16 * y); // placeholder
    return out;
}
...
function screenToGrid(x: number, y: number) {
    var out = new Vector2(Math.round(((x / 32) - 18.75 + (y / 16))/2), Math.round((18.75 + (y / 16) - (x / 32)) / 2)); // TODO: camera stuff needs to be taken into account, this might be the wrong spot for this function or something.
    return out;
}
